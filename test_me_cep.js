...
async function run() {
  ...
  const apiRes = await globalThis.fetch("https://melhorenvio.com.br/api/v2/me/shipment/agencies?company=1&zipcode=31910040", {
    headers: { "Authorization": "Bearer " + token, "Accept": "application/json" }
  });
  const data = await apiRes.json();
  if (data.data && data.data.length > 0) {
    console.log(data.data[0].address.city.state.state_abbr);
  }
  await client.end();
}
