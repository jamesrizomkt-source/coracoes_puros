...
async function run() {
  ...
  const apiRes = await globalThis.fetch("https://melhorenvio.com.br/api/v2/me/shipment/agencies?company=8&state=MG&city=Belo%20Horizonte", {
    headers: { "Authorization": "Bearer " + token, "Accept": "application/json" }
  });
  const data = await apiRes.json();
  console.log("Total Express Agencies:", data);
  await client.end();
}
