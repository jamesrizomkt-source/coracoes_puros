...
async function run() {
  ...
  await client.query(`
    ...
    DECLARE
      ...
      v_services jsonb;
    BEGIN
      -- 1. Get settings from individual rows
      ...
      -- Trim quotes if they exist (since value is jsonb in DB? No, value is text in settings, right?)
      -- Wait, if value is text, we don't need to strip quotes unless it was saved as JSON string.
      ...
      ELSE
        v_api_url := 'https://api.melhorenvio.com.br/api/v2/me/shipment/calculate';
      END IF;
      ...
      SELECT * INTO v_http_response FROM http((
        ...
        ARRAY[
          ...
          http_header('User-Agent', 'Antigravity Integration')
        ],
        'application/json',
        v_request_body::text
      )::http_request);
    ...
    $function$
  `);
  console.log("Updated RPC function calculate_melhor_envio");
  await client.end();
}
